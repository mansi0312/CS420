...
Proof.
  simpl.
  apply p_decidable_def with (p:=fun i => ACCEPT).
  apply p_decides_def.
  - simpl.
    apply p_recognizes_def.
    + simpl.
...
Proof.
intros. destruct (run_exists p) as (r, Hr).

Admitted.

(**

Hard. If a given program decides a language, then negating its result recognizes the complement of that language. Make sure you use p_decides_run_reject and p_decides_reject. In the second branch of proving recognizability, make sure you do a case analysis on the result of running p with i, that is to say, use:
